      ...
      <div ref={ref} className="wrap">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-16">
          ...
          <motion.div variants={up(1)} initial="hidden" animate={v ? "show" : "hidden"}>
            <div className="bg-white rounded-3xl p-8 md:p-10 border border-[#C9A86A]/8 h-full">
              <p className="font-heading text-[18px] text-[#111111] mb-1">Method 1</p>
              <p className="font-body text-[12px] text-[#C9A86A] tracking-[0.15em] uppercase mb-8">With Curd or Milk</p>
              <div className="space-y-6">
                {method1.map((step, i) => (
                  <div key={i} className="flex items-start gap-4">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#C9A86A] to-[#D4B97E] flex items-center justify-center flex-shrink-0 shadow-sm shadow-[#C9A86A]/20">
                      <span className="font-heading text-[13px] text-white">{i + 1}</span>
                    </div>
                    <p className="font-body text-[13px] text-[#111111]/50 leading-[1.8] pt-1.5">{step}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <motion.div variants={up(2)} initial="hidden" animate={v ? "show" : "hidden"}>
            <div className="bg-white rounded-3xl p-8 md:p-10 border border-[#C9A86A]/8 h-full">
              <p className="font-heading text-[18px] text-[#111111] mb-1">Method 2</p>
        ...
        <motion.div variants={up(3)} initial="hidden" animate={v ? "show" : "hidden"}>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {frequency.map((f) => (
              <div key={f.type} className="bg-white rounded-2xl p-6 border border-[#111111]/4 text-center">
